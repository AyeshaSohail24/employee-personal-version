...
/**
 * Edits ONE task already on ONE employee's currently launched onboarding plan instance.
 * Employee-specific only — mirrors AddTaskModal's boundary: this only ever touches THIS task
 * instance/its linked activity via onboardingService.updateTaskInInstance(), never the reusable
 * Onboarding Plans configuration, never another employee's plan.
 *
 ...
 * snapshotted Start Date Anchor (never a freshly resolved employee canonical date), matching
 * exactly how onboardingService.updateTaskInInstance() will calculate it on save.
 *
 ...
 */
export default function EditTaskModal({ isOpen, onClose, onSuccess, planInstanceId, task, employeeName, anchorDate }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setError(null);
    try {
      await onboardingService.updateTaskInInstance(planInstanceId, task.id, {
          ...
          <div className="modal-body modal-body-spacious">
            ...
            <div className="form-group" style={{ marginBottom: 0 }}>
              ...
              <div className="relative-timing-help">
                ...
                <ul>
                  <li><strong>0</strong> = On the Start Date anchor</li>
                  <li><strong>+ value</strong> = After the Start Date anchor (e.g., +3 = 3 days after)</li>
                  <li><strong>− value</strong> = Before the Start Date anchor (e.g., −3 = 3 days before)</li>
                </ul>
              </div>
