...
export default function OffboardingDashboardPage() {
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      // Fetch overdue activities that belong to Offboarding
      ...
      console.error('Failed to load offboarding dashboard data:', err);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="page-layout-container">
      ...
      <div className="onboarding-dashboard-header">
        <div className="header-text-group">
          <h1 className="page-title">Offboarding Dashboard</h1>
          <p className="page-subtitle">
            Manage offboarding plans, clearance workflows, and departing employee progress.
          </p>
        </div>
        <div className="header-actions">
          ...
          >
            <UserX size={15} />
            <span>Manage Exit Templates</span>
          </Link>
          <button
            type="button"
      ...
      <div className="onboarding-main-grid">
        {/* Left Column: Active Offboarding Workflows */}
        <div className="table-container-card">
          <div style={{ padding: '1rem 1.25rem', borderBottom: '1px solid var(--border-light)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <Link to="/offboarding/departing" style={{ fontSize: '0.815rem', color: 'var(--color-primary)', fontWeight: 600, display: 'inline-flex', alignItems: 'center', gap: '0.2rem' }}>
              View Departing Employees <ChevronRight size={14} />
            </Link>
          </div>

          {loading ? (
            <div style={{ padding: '2rem', textAlign: 'center', color: 'var(--text-muted)' }}>
              Loading active offboarding workflows...
            </div>
          ) : activeInstances.length === 0 ? (
            <div style={{ padding: '2.5rem', textAlign: 'center', color: 'var(--text-muted)' }}>
              No active offboarding plans running. Click "Launch Offboarding Plan" to start one.
            </div>
          ) : (
            <table className="presence-data-table" style={{ width: '100%' }}>
              <thead>
                <tr>
                  <th style={{ width: '28%', textAlign: 'left' }}>Departing Employee</th>
        ...
        {/* Right Column: Overdue Offboarding Tasks Alert List */}
        <div className="table-container-card" style={{ padding: '1rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', marginBottom: '1rem', borderBottom: '1px solid var(--border-light)', paddingBottom: '0.75rem' }}>
            ...
            <h3 style={{ margin: 0, fontSize: '0.95rem', fontWeight: 600, color: 'var(--text-main)' }}>
              Overdue Exit Tasks ({overdueTasks.length})
            </h3>
          </div>
          ...
          {overdueTasks.length === 0 ? (
            <div style={{ padding: '1.5rem 0', textAlign: 'center', color: '#059669', fontSize: '0.85rem' }}>
              ...
              <div>All exit clearance tasks are on schedule!</div>
            </div>
          ) : (
