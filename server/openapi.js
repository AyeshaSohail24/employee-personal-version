...
export const openapi = {
  ...
  paths: {
    ...
    "/onboarding/interns": {
      get: { summary: "Real interns whose Interns DB status is currently \"Onboarding\", cross-referenced with their local onboarding plan — auto-launching one for any intern who doesn't have one yet.", security: scoped("onboarding:read"),
        "x-rizurf": { name: "List Interns' Onboarding Progress", purpose: "The real interns actually in the Onboarding lifecycle stage, joined with whatever onboarding progress this app itself knows about each person", use_when: ["Showing an onboarding progress table"], do_not_use_when: ["You need every intern regardless of status — use GET /offboarding/interns or read through the Interns API directly", "You need this app's own employees, not interns — that's a separate, currently-empty roster"], inputs: [], outputs: ["interns[]"], requires: ["Filtered to the Interns DB's own status field equal to \"Onboarding\" (its enum: Onboarding/Active/Offboarding/Former) — not every intern, and not just anyone with a local plan already running", "No manual launch step: any real intern with no local plan yet has one auto-launched here from Universal + their department's active tasks, silently skipped if nothing is configured for their scope yet — see server/db/onboarding.js's listInternsWithAutoLaunchedOnboarding()"], related_endpoints: ["GET /onboarding/instances", "GET /onboarding/scope-tasks"], tags: ["onboarding", "interns", "progress", "roster", "auto-assign"] } },
    },
    ...
    "/offboarding/interns": {
      get: { summary: "Real interns whose Interns DB status is currently \"Offboarding\", cross-referenced with their local offboarding plan if one exists.", security: scoped("offboarding:read"),
        "x-rizurf": { name: "List Interns' Offboarding Progress", purpose: "The real interns actually in the Offboarding lifecycle stage, joined with whatever offboarding progress this app itself knows about each person", use_when: ["Showing an offboarding progress table"], do_not_use_when: ["You need every intern regardless of status — use GET /onboarding/interns or read through the Interns API directly", "You need this app's own employees, not interns — that's a separate, currently-empty roster"], inputs: [], outputs: ["interns[]"], requires: ["Filtered to the Interns DB's own status field equal to \"Offboarding\" (its enum: Onboarding/Active/Offboarding/Former) — not every intern approaching their end date, and not just anyone with a local plan already running"], related_endpoints: ["POST /offboarding/instances", "GET /offboarding/instances/{id}"], tags: ["offboarding", "interns", "progress", "roster"] } },
    },
    ...
    "/candidates/{applicantId}/messages": {
      get: { summary: "Read a candidate's full message thread.", security: scoped("candidate-messaging:read"),
        "x-rizurf": { name: "Get Candidate Thread", purpose: "Read the sent/received messages for one applicant. Also marks any unseen reply seen — opening a thread is what clears the notification badge", use_when: ["Opening a candidate's conversation view"], do_not_use_when: [], inputs: ["applicantId"], outputs: ["messages[]"], requires: ["direction: \"received\" rows are real replies matched by From-address against this applicant's email, polled from the same mailbox sendEmail uses (see server/messaging/imapReplyChecker.js, IMAP host/port from MAIL_IMAP_HOST/MAIL_IMAP_PORT) — a reply from an unrecognized address is read and marked seen but never recorded"], related_endpoints: ["POST /candidates/{applicantId}/messages", "GET /candidates"], tags: ["candidate", "messages", "email", "thread", "reply", "imap"] } },
      post: { summary: "Send a message to a candidate.", security: scoped("candidate-messaging:write"),
        "x-rizurf": { name: "Send Candidate Message", purpose: "Message a shortlisted applicant on email or WhatsApp", use_when: ["Replying to a candidate", "Sending an offer"], do_not_use_when: [], inputs: ["applicantId", "channel", "toEmail", "ccEmail", "toPhone", "subject", "body"], outputs: ["message"], requires: ["channel: \"email\" really delivers via SMTP (MAIL_* must be configured — see server/messaging/emailProvider.js) and throws if it fails to send", "channel: \"whatsapp\" has no real provider chosen yet — records the message but never delivers it (server/messaging/whatsappProvider.js is a stub)"], related_endpoints: ["GET /email-templates"], tags: ["candidate", "email", "whatsapp", "send", "offer", "channel"] } },
    },
    "/email-templates": {
    ...
    "/applicants/{applicantId}/convert": {
      post: { summary: "Accept an applicant: create them as an Onboarding intern in the Interns database, link a local employee record, and remove them from Upcoming.", security: scoped("applicant-conversion:write"),
        "x-rizurf": { name: "Convert Applicant", purpose: "Turn an accepted applicant into an employee, and move them to the `completed` phase in the real Recruitment API", use_when: ["A candidate in Upcoming (confirmation phase) is accepted"], do_not_use_when: ["The person isn't from Upcoming — use POST /employees directly"], inputs: ["applicantId", "startDate", "internshipEndDate", "icPassportNumber", "phone", "homeAddress", "departmentId", "mode", "allowance", "roleId", "photoUrl"], outputs: ["employee", "intern", "conversion", "phaseMoved"], requires: ["Applicant exists in the Applicants API (recruitment-api) at phase=confirmation", "Not already converted (422 if so)", "The intern is created in the Interns DB first; if that fails nothing else is written (422 with the reason)", "icPassportNumber/departmentId/roleId/homeAddress for an Intern hire — the Recruitment API only ever has name/email/phone/linkedin/github for an applicant"], related_endpoints: ["POST /employees", "GET /employees/{id}", "GET /positions"], tags: ["applicant", "convert", "hire", "accept"] } },
    },
